...
# --- force-register meta routes (place RIGHT AFTER app = Flask(...)) ---
import os, datetime
...
# ここで “必ず” ルートを登録する（デコレータを通らなくても有効）
...
def _inject_version_info():  # 右下バッジ用
    return dict(version_info=VERSION_INFO)

# デバッグ用の超単純ルート（確実に出るはず）
app.add_url_rule("/__test__", "__test__", lambda: ("__test__ ok", 200))
# ----------------------------------------------------------------------
